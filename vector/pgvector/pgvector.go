// Package pgvector - This is the Postgres-backed implementation of vector.Store using the pgvector
...
func New(ctx context.Context, opts Options) (*Store, error) {
	...
	if opts.EmbeddingDim <= 0 {
		return nil, errors.New("pgvector: embeddingDim must be greater than zero")
	}
	...
	if err := ensureExtension(ctx, opts.DSN); err != nil {
		return nil, fmt.Errorf("pgvector: install extension: %w", err)
	}
...
func (s *Store) migrate(ctx context.Context, dim int) error {
	...
	for _, q := range stmts {
		if _, err := s.pool.Exec(ctx, q); err != nil {
			return fmt.Errorf("pgvector: migrate %q: %w", firstLine(q), err)
		}
	}
...
//	if err != nil {
//		return fmt.Errorf("pgvector: begin transaction: %w", err)
//	}
...
//		if err != nil {
//			return fmt.Errorf("pgvector: marshal metadata %s: %w", d.ID, err)
//		}
...
// Upsert - optimized for bigger document loads
// inserts new rows or replaces existing ones by ID, in a single
...
func (s *Store) Upsert(ctx context.Context, docs []vector.Document) error {
	...
	if len(docs) == 1 {
		...
		_, err = tx.Exec(ctx, stmt, d.ID, d.Content, meta, pgvector.NewVector(d.Embedding))
		if err != nil {
			return err
		}
		return tx.Commit(ctx)
	...
	for _, d := range docs {
		...
		batch.Queue(stmt, d.ID, d.Content, meta, pgvector.NewVector(d.Embedding))
	}
...
func (s *Store) Query(ctx context.Context, embedding []float32, topK int) ([]vector.Result, error) {
	...
	const stmt = `
		SELECT id, content, metadata, embedding <=> $1 as distince
		FROM documents
...
// operations on the Store will fail silently
