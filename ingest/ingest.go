...
//
// At query time the user's question will be
...
func processContent(ctx context.Context, source string, content []byte, opts Options, embedder llm.Embedder, store vector.Store) (int, error) {
	...
	if !supportedFormat(base) {
		return 0, fmt.Errorf("unsupported file format: %s", filepath.Ext(base))
	}
	...
	if err != nil {
		return 0, fmt.Errorf("error embedding chunks: %w", err)
	}

	if len(vectors) != len(chunks) {
		return 0, fmt.Errorf("processContent: got %d vectors, for %d chunks", len(vectors), len(chunks))
	}
	...
	if err := store.Upsert(ctx, docs); err != nil {
		return 0, fmt.Errorf("error upserting documents: %w", err)
	}
...
func supportedFormat(path string) bool {
	...
	case ".txt", ".md", ".markdown", ".mdown", ".mark":
		return true
	}
	return false
}
